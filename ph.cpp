...
//#include "../AAA_ALEJANDRIA/Cpp_numeros.h"
...
int main(int argn, char **argc){
	...
	if(objetivo.es_valida()){
		ifstream is(objetivo.nombre_fichero(), ifstream::in);

		if(!is.fail()){
			if(argn>1) cout << "Tareas pendientes para el " << objetivo << ":" << endl;
			else cout << "Tareas pendientes para hoy:" << endl;
			is.getline(buffer,LEN_BUFFER,'\n');
			for(i=1; !is.fail(); ++i){
				cout << setw(2) << i << ": " << buffer << endl;
				is.getline(buffer,LEN_BUFFER,'\n');
			}
			is.close();
		}
		else{
			if(argn>1) cout << "No tienes nada pendiente para el " << objetivo << "." << endl;
			else cout << "No tienes nada pendiente para hoy." << endl;
		}
	}
